    ...
    def post(self, request):
        try:
            ...
            if user:
                ...
                return Response({'success': 'User logged in successfully', 'user_info': user_info}, status=status.HTTP_200_OK)

            else:
                return Response({'error': 'User not found'}, status=status.HTTP_400_BAD_REQUEST)

        except Exception as e:
            error = Error.objects.create(error=str(e))
            error.save()
            return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
class FirstLoginChangePasswordView(APIView):
    def post(self, request):
        try:
            ...
            repeatPassword = request.data.get('confirmPassword')
